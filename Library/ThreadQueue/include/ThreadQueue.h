 ...
 *              修复 ThreadQueueAPI_PutMsg 返回值文档错误
 * @author      zlzksrl
 */
...
/*                                                                    */
/*  使用流程:                                                         */
/*    ThreadQueueAPI_InitMsg() → ThreadQueueAPI_PutMsg() / ThreadQueueAPI_GetMsg() */
/*    → ThreadQueueAPI_CloseMsg() → ThreadQueueAPI_FlushMsg()                 */
/*    → ThreadQueueAPI_DestroyMsg()                                       */
/*                                                                    */
/*  错误码约定:                                                       */
/*     0  : 成功                                                      */
/*    -1  : 参数无效或未初始化                                         */
 ...
 *               如果队列已关闭，立即返回 -2。
 *               与 ThreadQueueAPI_PutMsg 的区别:
 *               - ThreadQueueAPI_PutMsg: 队列满时无限阻塞等待
 *               - ThreadQueueAPI_PutMsgTimeout: 队列满时带超时等待，超时后返回 -3
 * @param[in]    pt_QueueMsg: 队列结构体指针
 * @param[in]    data:        待发送的数据指针（不能为 NULL）
 ...
 *               关闭后:
 *               - ThreadQueueAPI_PutMsg() 将返回 -2
 *               - ThreadQueueAPI_GetMsg() 仍可读取队列中的剩余数据
 *               - 队列为空且关闭时，ThreadQueueAPI_GetMsg() 返回 NULL
 * @param[in]    pt_QueueMsg: 队列结构体指针
 * @return       int ret
 * @retval       0:    关闭成功
 * @retval       -1:   参数无效或队列未初始化
 * @warning      关闭操作不可逆（除非调用 ThreadQueueAPI_ReopenMsg）
 * @author       zlzksrl
 * @date         2026-05-07
 * @Version      V1.1.0
 */
 ...
 * @note         建议先调用 ThreadQueueAPI_FlushMsg 以便在销毁前自定义处理残留数据
 * @author       zlzksrl
 * @date         2026-05-07
 * @Version      V1.1.0
 */
...
/*                                                                    */
/*  使用流程:                                                         */
/*    ThreadQueueAPI_Latest_InitMsg() → ThreadQueueAPI_Latest_PutMsg()                       */
/*    / ThreadQueueAPI_Latest_GetMsg()                                            */
/*    → ThreadQueueAPI_Latest_CloseMsg() → ThreadQueueAPI_Latest_FlushMsg()                  */
/*    → ThreadQueueAPI_Latest_DestroyMsg()                                        */
/*                                                                    */
/*  错误码约定:                                                       */
/*     0  : 成功                                                      */
/*    -1  : 参数无效或未初始化                                         */
 ...
 *               关闭后 ThreadQueueAPI_Latest_PutMsg() 返回 -2。
 * @param[in]    pt_QueueMsg: 队列结构体指针
 * @return       int ret
 * @retval       0:    关闭成功
 * @retval       -1:   参数无效或队列未初始化
