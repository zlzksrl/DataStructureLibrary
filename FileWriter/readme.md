...
├── debug/
│   ├── main.c                        # 7 段演示：LOG/CSV/BIN/多实例/Rotate/查询/工具函数
...
| `FileWriter_DebugPro.bin` | 测试可执行程序（链接静态库） |
...
查询接口 GetXxx / StatsGet 并发              ← 任意业务线程调
   ↓
FileWriterAPI_Destroy()                      ← 主线程调，前提：所有业务线程已停用本 fw
```
...
| 任意接口 | `Destroy` | ❌ **不安全**——调用者保证 Destroy 时无并发 |

### Destroy 前的清理约定

```c
/* 正确姿势：先停业务线程，再 Destroy */
stop_business_threads();
FileWriterAPI_Destroy(&fw);

/* 错误姿势：业务线程还在 Write 时调 Destroy */
FileWriterAPI_Destroy(&fw);   // Write 可能拿到已释放的 SB → UAF
```
...
A: 当前只做 `fflush`（stdio buffer → kernel）。若需真正 flush 到 flash：V1.1 计划加 `fsync_ms` 配置字段。断电风险高的场景可暂时用 `Flush → sleep → Destroy` 组合。
